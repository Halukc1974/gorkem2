...
import { 
  Search, Network, Brain, Cpu, CircuitBoard, Sparkles, CheckCircle2, XCircle, Eye, FileText, Calendar, X 
} from 'lucide-react';
...
`;
  ...
  // Checkbox handlers for draft response feature
  const handleResponseCheckbox = (id: string) => {
    // Check if this document is already selected as reference
    if (selectedAsReference.has(id)) {
      toast({
        title: 'Warning',
    ...
    // Toggle selection
    setSelectedForResponse(id === selectedForResponse ? null : id);
  };

  const handleReferenceCheckbox = (id: string) => {
    // Check if this document is selected for response
    if (selectedForResponse === id) {
      toast({
        title: 'Warning',
    ...
    // Toggle in set
    const newSet = new Set(selectedAsReference);
    if (newSet.has(id)) {
      newSet.delete(id);
    } else {
      newSet.add(id);
    }
    setSelectedAsReference(newSet);
  };
  ...
  // Open draft configuration modal
  ...
  // Generate actual draft text using AI
  const handleActualDraftGeneration = async () => {
    ...
    try {
      ...
      const referenceDocs = analysisResults.filter(r => selectedAsReference.has(r.id));

      if (!responseDoc) {
        throw new Error('Response document not found');
      }

      const responseAnalysis = responseDoc.chatgptAnalysis || responseDoc.deepseekAnalysis;
      
      // Build prompt
      let prompt = `Based on the following document, generate a professional formal business response letter in English:\n\n`;
      prompt += `DOCUMENT TO RESPOND TO:\n`;
      prompt += `Letter No: ${responseDoc.letter_no}\n`;
      prompt += `Date: ${responseDoc.letter_date || 'N/A'}\n`;
      prompt += `Subject: ${responseDoc.short_desc || 'N/A'}\n`;
      if (responseAnalysis) {
        prompt += `Summary: ${responseAnalysis.summary}\n`;
        prompt += `Request: ${responseAnalysis.request}\n`;
        prompt += `Deadline: ${responseAnalysis.deadline || 'Not specified'}\n`;
      }

      if (referenceDocs.length > 0) {
        prompt += `\n\nREFERENCE DOCUMENTS:\n`;
        referenceDocs.forEach((doc, idx) => {
          const analysis = doc.chatgptAnalysis || doc.deepseekAnalysis;
          prompt += `${idx + 1}. Letter No: ${doc.letter_no}, Date: ${doc.letter_date || 'N/A'}\n`;
          if (analysis) {
      ...
      prompt += `\n\nGenerate a complete formal business letter that:\n`;
      prompt += `1. Acknowledges receipt of the referenced document\n`;
      prompt += `2. Addresses the main requests clearly\n`;
      prompt += `3. Proposes concrete solutions or next steps\n`;
      prompt += `4. Maintains a highly professional and formal tone\n`;
      prompt += `5. Is properly structured with date, subject, body, and closing\n`;
      prompt += `6. Includes proper salutations and sign-off\n`;
      if (referenceDocs.length > 0) {
        prompt += `7. References the provided reference documents where relevant\n`;
      }
      prompt += `\nReturn the complete draft letter in plain text format. Use proper business letter formatting.`;

      // Use selected AI model from analysisOptions
      ...
      if (analysisOptions.useChatGPT && config?.apis?.openai) {
        ...
        model = analysisOptions.chatgptModel;
      } else if (analysisOptions.useDeepSeek && config?.apis?.deepseek) {
        apiKey = config.apis.deepseek;
        apiUrl = 'https://api.deepseek.com/v1/chat/completions';
        model = 'deepseek-chat';
      } else {
        throw new Error('No AI model selected or API key not configured');
      }
      ...
      const response = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a professional business correspondence writer specializing in formal business letters.'
            },
            {
              role: 'user',
              content: prompt
      ...
      if (!draftText) {
        throw new Error('No draft text generated');
      }

      setGeneratedDraftText(draftText);
      setShowGeneratedDraftModal(true);
    ...
    Document Analysis
                          ...
                          <CardHeader className="pb-3">
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                ...
                                <div className="flex gap-6 ml-0 mt-2">
                                  <label className="flex items-center gap-2 cursor-pointer">
                                    <input
                                      type="checkbox"
                                      checked={selectedForResponse === result.id}
                                      onChange={() => handleResponseCheckbox(result.id)}
                                      className="w-4 h-4 rounded"
                                    />
                                    <span className="text-sm font-medium text-blue-700">For Response</span>
                                  </label>
                                  <label className="flex items-center gap-2 cursor-pointer">
                                    <input
                                      type="checkbox"
                                      checked={selectedAsReference.has(result.id)}
                                      onChange={() => handleReferenceCheckbox(result.id)}
                                      className="w-4 h-4 rounded"
                                    />
                                    <span className="text-sm font-medium text-green-700">Add as Reference</span>
                                  </label>
                                </div>
                              </div>
                            </div>
      ...
      <Dialog open={showDraftModal} onOpenChange={setShowDraftModal}>
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Generate Draft Response</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            {/* AI Model Selection */}
            <div className="border rounded-lg p-4 bg-gray-50">
              <h3 className="font-semibold text-sm mb-3">AI Model Selection</h3>
              <div className="space-y-3">
                {/* ChatGPT Section */}
                <div className="border rounded-lg p-3 space-y-3 bg-white">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-medium">ChatGPT (OpenAI)</div>
                        <div className="text-xs text-gray-500">Select model for draft generation</div>
                      </div>
                    </div>
                    <input
                      type="checkbox"
                      ...
                      onChange={(e) => setAnalysisOptions(prev => ({ 
                        ...prev, 
                        useChatGPT: e.target.checked,
                        useDeepSeek: e.target.checked ? false : prev.useDeepSeek
                      }))}
                      disabled={!config?.apis?.openai}
                      className="w-5 h-5 rounded"
                    />
                  ...
                  {analysisOptions.useChatGPT && (
                    <div className="pl-8 space-y-2">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="radio"
                          name="chatgpt-model-draft"
                          checked={analysisOptions.chatgptModel === 'gpt-4o-mini'}
                          onChange={() => setAnalysisOptions(prev => ({ ...prev, chatgptModel: 'gpt-4o-mini' }))}
                          className="w-4 h-4"
                        />
                        <span className="text-sm">GPT-4o-mini <span className="text-xs text-gray-500">(Default, 1x cost)</span></span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="radio"
                          name="chatgpt-model-draft"
                          checked={analysisOptions.chatgptModel === 'gpt-4o'}
                          onChange={() => setAnalysisOptions(prev => ({ ...prev, chatgptModel: 'gpt-4o' }))}
                          className="w-4 h-4"
                        />
                        <span className="text-sm">GPT-4o <span className="text-xs text-gray-500">(15x cost)</span></span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="radio"
                          name="chatgpt-model-draft"
                          checked={analysisOptions.chatgptModel === 'gpt-4-turbo'}
                          onChange={() => setAnalysisOptions(prev => ({ ...prev, chatgptModel: 'gpt-4-turbo' }))}
                          className="w-4 h-4"
                        />
                        <span className="text-sm">GPT-4-turbo <span className="text-xs text-gray-500">(30x cost)</span></span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="radio"
                          name="chatgpt-model-draft"
                          checked={analysisOptions.chatgptModel === 'gpt-3.5-turbo'}
                          onChange={() => setAnalysisOptions(prev => ({ ...prev, chatgptModel: 'gpt-3.5-turbo' }))}
                          className="w-4 h-4"
                        />
                        <span className="text-sm">GPT-3.5-turbo <span className="text-xs text-gray-500">(0.3x cost)</span></span>
                      </label>
                    </div>
                  )}
                </div>
                ...
                <div className="border rounded-lg p-3 bg-white">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-medium">DeepSeek</div>
                        <div className="text-xs text-gray-500">Alternative AI model</div>
                      </div>
                    </div>
                    <input
                      type="checkbox"
                      ...
                      onChange={(e) => setAnalysisOptions(prev => ({ 
                        ...prev, 
                        useDeepSeek: e.target.checked,
                        useChatGPT: e.target.checked ? false : prev.useChatGPT
                      }))}
                      disabled={!config?.apis?.deepseek}
                      className="w-5 h-5 rounded"
                    />
            ...
            {selectedForResponse && (() => {
              ...
              return (
                <div className="border rounded-lg p-4 bg-blue-50">
                  <h3 className="font-semibold text-sm mb-2">Document to Respond To:</h3>
                  <div className="space-y-1 text-sm">
                    <div><span className="font-medium">Document No:</span> {responseDoc.letter_no}</div>
                    <div><span className="font-medium">Date:</span> {responseDoc.letter_date}</div>
                    <div><span className="font-medium">Subject:</span> {responseDoc.short_desc}</div>
                  </div>
                </div>
              );
            })()}
            ...
            {/* Reference Documents */}
            {selectedAsReference.size > 0 && (
              <div className="border rounded-lg p-4 bg-green-50">
                <h3 className="font-semibold text-sm mb-2">Reference Documents:</h3>
                <div className="space-y-2">
                  {Array.from(selectedAsReference).map(refId => {
                    const refDoc = analysisResults.find(r => r.id === refId);
                    if (!refDoc) return null;
                    return (
                      <div key={refId} className="flex items-start justify-between bg-white p-2 rounded border">
                        <div className="text-sm flex-1">
                          <div><span className="font-medium">Document No:</span> {refDoc.letter_no}</div>
                          <div className="text-xs text-gray-600">{refDoc.letter_date} • {refDoc.short_desc}</div>
                        </div>
                        <button
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Example: Write a positive response, accept the requests... or Write a negative response, explain the reasons..."
              ...
              <p className="text-xs text-gray-500 mt-1">
                Optional: Specify how you want the response to be (positive/negative, detailed/brief, etc.)
              </p>
            </div>
            ...
            <div className="flex justify-end pt-4">
              <Button 
                onClick={handleActualDraftGeneration}
                disabled={!analysisOptions.useChatGPT && !analysisOptions.useDeepSeek}
                className="bg-green-600 hover:bg-green-700"
              >
                <FileText className="w-4 h-4 mr-2" />
                Generate Draft Response
      ...
      {/* Generated Draft Display Modal */}
      <Dialog open={showGeneratedDraftModal} onOpenChange={setShowGeneratedDraftModal}>
        <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Generated Draft Response</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            {/* Draft Text Display */}
            ...
            <div className="flex gap-3 justify-end">
              ...
              >
                <FileText className="w-4 h-4 mr-2" />
                Download as PDF (requires npm install jspdf)
              </Button>
            </div>
          </div>
        </DialogContent>
